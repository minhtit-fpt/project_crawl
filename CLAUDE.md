...
1. **CLI Mode**: Accepts a list of target website URLs and product SKUs (from YAML file or CMS API), extracts prices using Scrapy + Playwright, outputs structured results to the terminal, and persists to SQLite.
...
main.py --source [yaml|api]
  ...
  → scraper/scheduler.py             # Expand sites × SKUs into CrawlJob list
      ...
      → scraper/storage/database.py  # Persist results to SQLite
```
...
main.py --serve
  ...
  → scraper/storage/database.py      # Init SQLite schema
  → scraper/api_server.py            # FastAPI server
      ├─ GET /prices?sku=X           → Query SQLite for prices
...
- **database.py** — SQLite repository for crawl results and run metadata.
...
| `SQLITE_DB_PATH` | `data/crawl_results.db` | SQLite database file path |
| `DATABASE_URL` | (none) | Reserved for future use |
